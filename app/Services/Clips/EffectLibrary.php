...
class EffectLibrary
{
    ...
    public const BUILTIN_SAMPLES = [
        ...
        'seal-stamp' => ['label' => 'Seal stamp', 'text' => 'Brand Machine', 'params' => []],
...
TSX;
    ...
    private function renderIndex(Collection $active): string
    {
        ...
        // Generated effects are TOLD to lay themselves out from the `frame` prop, but
        // some ignore it and hardcode the portrait canvas — in a half frame they then
        // render at full height and get clipped (the reader only sees the top third).
        // Flag those so the renderer can scale them to fit instead of cropping them.
        // ponytail: detects the observed failure — the portrait height as a literal
        // (incl. SVG viewBox). An effect hardcoded to some other size slips through;
        // tighten only if that shows up.
        $c = config('contentmachine.clips');
        $stageW = (int) ($c['width'] ?? 1080);
        $stageH = (int) ($c['height'] ?? 1920);
        $locked = $active
            ->filter(fn (EffectRecord $e) => (bool) preg_match('/(?<![0-9])'.$stageH.'(?![0-9])/', (string) $e->tsx))
            ->map(fn (EffectRecord $e) => "\"{$e->slug}\"")
            ->implode(', ');

        return "{$header}\n".($imports !== '' ? $imports."\n" : '')."\n{$note}\n"
            ."export const CUSTOM_PRIMITIVES: Record<string, React.FC<PrimitiveProps>> = {$body};\n"
            ."// The canvas a portrait-locked effect assumes, and the effects that assume it.\n"
            ."export const PORTRAIT_STAGE = { width: {$stageW}, height: {$stageH} };\n"
            ."export const PORTRAIT_LOCKED: ReadonlySet<string> = new Set([{$locked}]);\n";
    }
